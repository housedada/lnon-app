  ...
  return (
    <div>
      ...
      <Suspense fallback={<ListPlaceholder />}>
        <InvoicesListSection params={params} isSuperadmin={isSuperadmin} />
      </Suspense>
    </div>
  );
}

async function InvoicesListSection({ params, isSuperadmin }: { params: SearchParams; isSuperadmin: boolean }) {
  const { q, page } = params;
  ...
  return (
    <ListNavigator
      ...
      totalCount={total}
      totalLabel="fatture"
      extraTopControls={<InvoicesBulkBar invoiceGroupKeys={invoiceGroupKeys} />}
    >
      <div className="mx-6 mt-6 overflow-x-auto border-t border-grid-border">
        <div className="grid w-full text-[12px]" style={{ gridTemplateColumns: '32px repeat(8, minmax(max-content, 1fr)) max-content' }}>
          ...
          {invoices.map((invoice) => (
            <ProjectInvoiceRow key={invoice.id} invoice={invoice} isSuperadmin={isSuperadmin} />
          ))}
        </div>
      </div>
    </ListNavigator>
