...
import { confirmJobClientMatchesAction, type JobClientMatchSuggestion } from '@/lib/actions/jobs';
import { notify } from '@/lib/notify';

export default function JobClientMatchModal({
  suggestions,
  onClose,
}: {
  suggestions: JobClientMatchSuggestion[];
  onClose: () => void;
}) {
  const [selected, setSelected] = useState<Set<string>>(new Set(suggestions.map((s) => s.jobId)));
  ...
  function handleConfirm() {
    const pairs = suggestions.filter((s) => selected.has(s.jobId)).map((s) => ({ jobId: s.jobId, clientId: s.clientId }));
    startTransition(async () => {
      const count = await confirmJobClientMatchesAction(pairs);
      notify(`${count} lavori collegati al cliente corrispondente.`);
      router.refresh();
      onClose();
    });
  }
    ...
    <div className="modal-backdrop fixed inset-0 z-[200] flex items-center justify-center bg-black/50 p-4" onClick={isPending ? undefined : onClose}>
      ...
      >
        <div className="flex items-start justify-between gap-3">
          <div>
            <h2 id="job-match-modal-title" className="text-base font-semibold text-primary">
              Verifica abbinamenti lavoro → cliente
            </h2>
            <p className="mt-1 text-sm text-secondary">
              Il nome cliente indicato nel lavoro corrisponde a un cliente in anagrafica. Controlla e conferma solo
              quelli corretti.
            </p>
          </div>
        ...
        <ul className="mt-5 divide-y divide-grid-border rounded-lg border border-grid-border">
          {suggestions.map((s) => (
            <li key={s.jobId} className="flex items-center gap-3 px-3 py-2.5 text-sm">
              <input type="checkbox" checked={selected.has(s.jobId)} onChange={() => toggle(s.jobId)} aria-label={`Abbina ${s.jobClientName}`} />
              <span className="flex-1 text-primary">{s.jobClientName}</span>
