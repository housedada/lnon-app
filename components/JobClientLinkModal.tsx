...
import { linkJobToClientAction } from '@/lib/actions/jobs';
import { notify } from '@/lib/notify';

export default function JobClientLinkModal({
  jobId,
  jobClientName,
  clientOptions,
  onClose,
  ...
  function handleLink(clientId: string) {
    ...
    startTransition(async () => {
      await linkJobToClientAction(jobId, clientId);
      notify('Lavoro collegato al cliente.');
      router.refresh();
      onClose();
    });
  }
  ...
  return createPortal(
    <div className="modal-backdrop fixed inset-0 z-[200] flex items-center justify-center bg-black/50 p-4" onClick={isPending ? undefined : onClose}>
      <div
        role="dialog"
        aria-modal="true"
        aria-labelledby="job-link-modal-title"
        className="modal-panel card-shadow w-full max-w-md rounded-xl border border-grid-border bg-card-bg p-6"
        onClick={(e) => e.stopPropagation()}
      >
        <div className="flex items-start justify-between gap-3">
          <div>
            <h2 id="job-link-modal-title" className="text-sm font-semibold text-primary">
              Collega a un cliente
            </h2>
            <p className="mt-1 text-xs text-secondary">Lavoro: {jobClientName}</p>
          </div>
