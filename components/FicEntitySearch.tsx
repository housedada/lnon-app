...
export default function FicEntitySearch({
  placeholder,
  redirectTo,
  searchAction,
  linkAction,
  ...
  placeholder: string;
  redirectTo: string;
  searchAction: (query: string) => Promise<EntitySummary[]>;
  ...
  return (
    <div className="mt-3">
      ...
      {!isSearching && searched && results.length === 0 && (
        <p className="mt-3 text-xs text-secondary">Nessun risultato trovato su Fatture in Cloud.</p>
      )}
            ...
            return (
              <li key={r.id} className="flex items-center justify-between px-3 py-2 text-sm">
                <div>
                  ...
                  {r.subtitle && <p className="text-xs text-secondary">{r.subtitle}</p>}
                </div>
                <button
                  type="button"
